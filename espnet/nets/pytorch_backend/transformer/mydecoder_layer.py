    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, cache=None):
        ...
        if cache is None:
            ...
            assert cache.shape == (
                ...
                dec.shape[1] - 1,
                self.size,
            ), f"{cache.shape} == {(dec.shape[0], dec.shape[1] - 1, self.size)}"
            dec_q = dec[:, -1:, :]
            residual_dec = residual_dec[:, -1:, :]
            tgt_q_mask = None
        ...
        # print(enc.size())
        # print(memory_mask.size())
        enc = self.self_attn(enc, enc, enc, memory_mask)

        # enc , dec mixed attention part
        # 參數應該會吃到enc跟dec跟 token 第幾個這樣m or 看mask可以知道
        # q,k,v,mask
        dec = self.mixed_attn(dec, torch.cat((enc, dec), 1), torch.cat((enc, dec), 1), tgt_mask)
