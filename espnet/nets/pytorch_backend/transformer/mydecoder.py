...
# use my attention and decoder layer
# from espnet.nets.pytorch_backend.transformer.attention import MultiHeadedAttention
# from espnet.nets.pytorch_backend.transformer.decoder_layer import DecoderLayer

from espnet.nets.pytorch_backend.transformer.myattention import MultiHeadedAttention
from espnet.nets.pytorch_backend.transformer.myattention import MultiHeadedMixedAttention
from espnet.nets.pytorch_backend.transformer.mydecoder_layer import DecoderLayer
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def __init__(
        ...
        mixed_attention_dropout_rate=0.0,
        input_layer="embed",
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask):
        ...
        """
        x = self.embed(tgt)
        # x 是 label (decode target) memory 是 encoder output
        ...
        concat_enc_dec = torch.cat((memory,x),1)
        concat_enc_dec = self.after_norm(concat_enc_dec)

        memory = concat_enc_dec[:,:n,:]
        x = concat_enc_dec[:,n:,:]
