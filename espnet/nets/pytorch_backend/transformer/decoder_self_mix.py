...
from espnet.nets.pytorch_backend.transformer.decoder_layer_self_mix import DecoderLayer
...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask):
        ...
        """
        x = self.embed(tgt)
        # print('x',x.shape,'tgt_mask',tgt_mask.shape,'memory',memory.shape,'memory_mask',memory_mask.shape)
        ...
        # ctc2
        length = x.shape[1]
        input_1 = torch.cat((x, memory), dim=1)
        if self.normalize_before:
            input_1 = self.after_norm(input_1)
        x = input_1[:,:length,:]
        memory = input_1[:,length:,:]
