...
from espnet.nets.pytorch_backend.transformer.mydecoder import Decoder
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.decoder is not None:
            ...
            pred_pad, pred_mask , memory , memory_mask = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
        ...
        if self.mtlalpha == 0.0:
            ...
            loss_ctc = self.ctc(memory.view(batch_size, -1, self.adim), hs_len, ys_pad)
            if not self.training and self.error_calculator is not None:
                ys_hat = self.ctc.argmax(memory.view(batch_size, -1, self.adim)).data
                cer_ctc = self.error_calculator(ys_hat.cpu(), ys_pad.cpu(), is_ctc=True)
            # for visualization
            if not self.training:
                self.ctc.softmax(memory)
